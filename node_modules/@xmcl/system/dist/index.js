...
__export(system_exports, {
  ...
  resolveFileSystem: () => resolveFileSystem
});
...
// index.ts
var access = (0, import_util.promisify)(import_fs.access);
var stat = (0, import_util.promisify)(import_fs.stat);
var writeFile = (0, import_util.promisify)(import_fs.writeFile);
var readFile = (0, import_util.promisify)(import_fs.readFile);
var readdir = (0, import_util.promisify)(import_fs.readdir);
async function openFileSystem(basePath) {
  if (typeof basePath === "string") {
    ...
    if (fstat.isDirectory()) {
      ...
      const zip = await (0, import_unzip.open)(basePath);
      Object.assign(zip, {
        validateFileName: (e) => {
      ...
      const entries = await (0, import_unzip.readAllEntries)(zip).then(
        (es) => (
    ...
    const zip = await (0, import_unzip.open)(basePath);
    const entries = await (0, import_unzip.readAllEntries)(zip);
    const entriesRecord = {};
    for (const entry of entries) {
      entriesRecord[entry.fileName] = entry;
    }
...
var NodeFileSystem = class extends FileSystem {
  ...
  writeable = true;
  join(...paths) {
    return (0, import_path.join)(...paths);
  }
  getUrl(name) {
    return `file://${this.join(this.root, name)}`;
  }
  isDirectory(name) {
    return stat((0, import_path.join)(this.root, name)).then((s) => s.isDirectory());
  }
  writeFile(name, data) {
    return writeFile((0, import_path.join)(this.root, name), data);
  }
  existsFile(name) {
    return access((0, import_path.join)(this.root, name)).then(() => true, () => false);
  }
  readFile(name, encoding) {
    return readFile((0, import_path.join)(this.root, name), { encoding });
  }
  listFiles(name) {
    return readdir((0, import_path.join)(this.root, name));
  }
  cd(name) {
    this.root = (0, import_path.join)(this.root, name);
  }
};
var NodeZipFileSystem = class extends FileSystem {
  ...
  async readFile(name, encoding) {
    ...
    const buffer = await (0, import_unzip.readEntry)(this.zip, entry);
    if (encoding === "utf-8") {
  ...
  listFiles(name) {
    ...
    return Promise.resolve([
      ...new Set(Object.keys(this.entries).filter((n) => n.startsWith(name)).map((n) => n.substring(name.length)).map((n) => n.startsWith("/") ? n.substring(1) : n).map((n) => n.split("/")[0]))
    ]);
  }
  cd(name) {
    if (name.startsWith("/")) {
